...
`
	...
	cmd := exec.Command("../maestro", "vdb", "create", tempFile, "--dry-run")
...
`
	...
	cmd := exec.Command("../maestro", "vdb", "create", tempFile, "--uri=localhost:8000", "--mode=remote", "--verbose", "--dry-run")
	output, err := cmd.CombinedOutput()
...
func TestCreateVectorDatabaseWithInvalidResourceType(t *testing.T) {
	cmd := exec.Command("../maestro", "vdb", "invalid-action", "test.yaml")
...
func TestCreateVectorDatabaseWithNonExistentFile(t *testing.T) {
	cmd := exec.Command("../maestro", "vdb", "create", "nonexistent.yaml")
	output, err := cmd.CombinedOutput()
...
`
	...
	cmd := exec.Command("../maestro", "vdb", "create", tempFile)
	output, err := cmd.CombinedOutput()
...
`
	...
	cmd := exec.Command("../maestro", "vdb", "create", tempFile, "--silent", "--dry-run")
	output, err := cmd.CombinedOutput()
