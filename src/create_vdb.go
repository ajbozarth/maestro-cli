...
Usage:
  maestro create vector-database YAML_FILE [options]

Examples:
  maestro create vector-database config.yaml
  maestro create vector-database config.yaml --uri=localhost:8000 --mode=local`,
...
Usage:
  maestro create vector-db YAML_FILE [options]

Examples:
  maestro create vector-db config.yaml
  maestro create vector-db config.yaml --uri=localhost:8000 --mode=local`,
	Args: cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		// Suppress usage for all errors except usage errors
		cmd.SilenceUsage = true
...
Usage:
  maestro create vdb YAML_FILE [options]

Examples:
  maestro create vdb config.yaml
  maestro create vdb config.yaml --uri=localhost:8000 --mode=local`,
	Args: cobra.ExactArgs(1),
	RunE: func(cmd *cobra.Command, args []string) error {
		// Suppress usage for all errors except usage errors
		cmd.SilenceUsage = true
